MODULE MOD_MEND_TYPE    
    ...
    INTEGER, PARAMETER:: const_nPOOL_MN     = 20                    !# of state variables in sMEND_POOL_MN   !2023/8/12 added
    ...
    REAL(8), PARAMETER:: const_soilDepth = 100.d0                      !mark0306
    ! REAL(8), PARAMETER:: const_Kt = 0.57                             ![W/m/K] soil thermal condutivity for unfrozen soil
    ! REAL(8), PARAMETER:: const_th = 86400                            ![s/day] time length
    ! REAL(8), PARAMETER:: const_Lf = 335000                           ![J/kg] latent heat of fusion of ice
    ! REAL(8), PARAMETER:: const_ICEdensity = 971                      ![kg/m3] ice density
    ! REAL(8), PARAMETER:: const_ALT0 = 0.5                            ![cm] minimum ALT
    ! REAL(8), PARAMETER:: const_SWCavg = 0.22                         ![m3/m3] average for SWC input (0, 1) mark0201
    
    character(len=4), PARAMETER:: const_obj_name(9) = (/"NSEC","NSEn","MARE","MARt", "MARn", "CORR", "CORl", "AVGr","Rang"/) !2023/8/12 added
    ...
    TYPE sMEND_POOL_MN
        ...
        REAL(8) MBC_SOC                    !2023/8/12 added
        REAL(8) ENZ_MBC                    !2023/8/12 added
        REAL(8) Fdepth                     !frozen depth (mark0303)
        REAL(8) Tdepth                     !thaw depth (mark0303)
        REAL(8) ALT_scalar                 !(mark0303)
    ...
    TYPE sMEND_PAR
        ...
        REAL(8) frMB2POMO                  ![-], fraction of dead MMB to Oxidative POM
        ...
        REAL(8) YgN                        ![-],Nitrogen Use Index (NUI), the fraction of N uptake being incorporated into microbial biomass
        REAL(8) wNUI                       ![-],exponent to compute NUI, i.e., YgN

        REAL(8) Qmax_NH4                   ![mg N/cm3], NH4 adsorption capacity
        REAL(8) Kba_NH4                    ![1/(mg N/cm3)], NH4 adsorption binding affinity
         
        REAL(8) KsNH4_VG                   ![mg N/cm3], half-saturation constant for NH4 uptake by plant (VG)
        REAL(8) KsNO3_VG                   ![mg N/cm3], half-saturation constant for NO3 uptake by plant (VG)
        ...
        REAL(8) ALTa                        !fALT_A2D, fALT_D2A mark0202
        REAL(8) ALTb                        !min ALT mark1105
        REAL(8) ALTc
    ...
    TYPE sMEND_INP
        ...
        REAL(8) SIN3                                   ![mgC/cm3/h],type3 SOC input, e.g., roots
        REAL(8) tmp                                    ![C],temperature
        REAL(8) Fdep                                   ![cm]frozen depth (mark0303)
        REAL(8) Tdep                                   ![cm]thaw depth (mark0303)
        REAL(8) pH                                     ![-],pH value
    ...
    TYPE sMEND_INI
        ...
        INTEGER iALT_type                              !0: non-frozen soil; 1: permafrost; 2: seasonally frozen soil
        ...
        REAL(8) Kt                                     ![W/m/deg = W/m/K], soil thermal conductivity of thawing soils, for ALT
        REAL(8) th                                     ![s/day], time length, for ALT
        REAL(8) Lf                                     ![J/kg], latent heat of fusion, 3.34*10**5, for ALT
        REAL(8) ICEdensity                             ![kg/m3], ice density, 0.917*10**3, for ALT
        REAL(8) ALT0                                   ![cm], minimum ALT to represent decomposition during winters
        ...
        REAL(8), ALLOCATABLE:: SIN(:)                  ![mgC/cm3/h],SOC input, e.g., litterfall
        REAL(8), ALLOCATABLE:: SIN3(:)                 ![mgC/cm3/h],type3 SOC input, e.g., roots
        ...
        REAL(8) SIN3_frac(3)                           !fraction of type3 SOC input to 3 pools (POM1, POM2, & DOC)
