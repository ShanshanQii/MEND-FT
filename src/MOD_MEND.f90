MODULE MOD_MEND  
    ...
    PUBLIC:: fALT_DEC
    PUBLIC:: fALT_A2D
    PUBLIC:: fALT_D2A !qss:2024/02/02
...
CONTAINS
    ...
    !-----------------------------------------------------------------------------
    REAL(8) FUNCTION fCN_UPDATE(CPOOL, NPOOL, CN0)
        !! Calculate C:N ratios in SOM pools
        REAL(8) CPOOL, NPOOL, CN0
            ...
            if(sINI % iModel.eq.4) then !! MCMC
                ...
                if(trim(sINI%VARobj(j)).eq."NSEC") then
                    ...
                    sINI%rOBJ(i) = fSIGN(sINI%VARopt_int(i,2), sINI%dOBS_opt(ibeg:iend,2), sINI%dSIM_opt(ibeg:iend,2), &
                        sINI%VARobj_tol(j), const_FillValue)
                else if (trim(sINI%VARobj(j)).eq."Rang") then !! frang
                    sINI%rOBJ(i) = fRANG(sINI%VARopt_int(i,2), sINI%dOBS_opt(ibeg:iend,2), sINI%dSIM_opt(ibeg:iend,2), &
                        sINI%VARobj_tol(j), const_FillValue)
                else !!
    ...
    SUBROUTINE subFreezeThawFront_Generate(sINI)
        ...
        REAL(8), ALLOCATABLE :: SWC_phase(:), Fdep_final(:), Tdep_final(:)
        LOGICAL, ALLOCATABLE :: is_thaw(:)
        REAL(8) :: FT_swc_sat, FT_half_sand, FT_BD_kgm3

        if (sINI%iALT_type .eq. 0) then
            sINI%Fdep = const_FillValue
            sINI%Tdep = const_FillValue
            return
        else if (sINI%iALT_type .lt. 1 .or. sINI%iALT_type .gt. 2) then
            print*,"Freeze-thaw error: invalid iALT_type = ",sINI%iALT_type
            stop
        end if

        FT_swc_sat = sINI%porosity
        FT_half_sand = sINI%fTexture(1) / 2.d0
        !! fFT_get_kt/fFT_get_kf use the bulk-density numerical convention kg m-3.
        FT_BD_kgm3 = 2650.d0 * (1.d0 - sINI%porosity)

        if (FT_swc_sat .le. 0.d0 .or. FT_swc_sat .ge. 1.d0) then
            print*,"Freeze-thaw error: porosity must be between 0 and 1; value = ",FT_swc_sat
            stop
        end if
        if (sINI%fTexture(1) .lt. 0.d0 .or. sINI%fTexture(1) .gt. 1.d0) then
            print*,"Freeze-thaw error: Sand fraction must be between 0 and 1; value = ",sINI%fTexture(1)
            stop
        end if
        if (FT_BD_kgm3 .le. 0.d0) then
            print*,"Freeze-thaw error: derived bulk density must be positive; value (kg m-3) = ",FT_BD_kgm3
            stop
        end if
        ...
        ALLOCATE(STP_day(ndays), SWC_day(ndays), STP_smooth(ndays), SWC_phase(ndays))
        ALLOCATE(Fdep_final(ndays), Tdep_final(ndays), is_thaw(ndays))
        ...
        CALL subFT_BuildThawMask(sINI%sDate_beg_all, STP_smooth, is_thaw)
        CALL subFT_FillPhaseMeanSWC(SWC_day, is_thaw, FT_swc_sat, SWC_phase)

        SELECT CASE (sINI%iALT_type)
        CASE (1)
            CALL subFT_ComputePermafrostFronts(STP_smooth, SWC_phase, is_thaw, &
                FT_swc_sat, FT_half_sand, FT_BD_kgm3, Fdep_final, Tdep_final)
        CASE (2)
            CALL subFT_ComputeSeasonalFronts(STP_smooth, SWC_phase, is_thaw, &
                FT_swc_sat, FT_half_sand, FT_BD_kgm3, Fdep_final, Tdep_final)
        END SELECT
        ...
        DEALLOCATE(STP_day, SWC_day, STP_smooth, SWC_phase)
        DEALLOCATE(Fdep_final, Tdep_final, is_thaw)
    ...
    !-----------------------------------------------------------------------------
    SUBROUTINE subFT_BuildThawMask(sDate_beg, STP_day, is_thaw)
        CHARACTER(LEN=8), intent(in) :: sDate_beg
        REAL(8), intent(in) :: STP_day(:)
        LOGICAL, intent(out) :: is_thaw(size(STP_day))
        ...
        !! Preserve the existing thaw-season detection semantics exactly.
        !! The thaw-start counter is cumulative within each year and is not reset
        !! by an intervening negative-temperature day before it reaches seven.
        ibeg = 1
        do while (ibeg .le. n)
            iend = ibeg
            do while (iend .lt. n)
                if (years(iend + 1) .ne. years(ibeg)) exit
                iend = iend + 1
            end do
            ...
            count = 0
            do i = ibeg, iend
                if (STP_day(i) .ge. 0.d0) count = count + 1
                if (count .eq. 7) then
                    thaw_start = i - 6
                    exit
                end if
            ...
            if (thaw_start .gt. 0) then
                count = 0
                do i = thaw_start + 1, iend
                    if (STP_day(i) .lt. 0.d0) then
                        count = count + 1
                    else
                        count = 0
                    end if
                ...
                if (thaw_end .eq. 0) thaw_end = iend + 1
                is_thaw(thaw_start:min(thaw_end - 1, iend)) = .true.
            end if
        ...
        DEALLOCATE(years)
    END !!subFT_BuildThawMask

    !-----------------------------------------------------------------------------
    SUBROUTINE subFT_FillPhaseMeanSWC(SWC_day, is_thaw, swc_sat, SWC_phase)
        REAL(8), intent(in) :: SWC_day(:), swc_sat
        LOGICAL, intent(in) :: is_thaw(size(SWC_day))
        REAL(8), intent(out) :: SWC_phase(size(SWC_day))
        ...
        do while (ibeg .le. n)
            iend = ibeg
            do while (iend .lt. n)
                if (.not. (is_thaw(iend + 1) .eqv. is_thaw(ibeg))) exit
                iend = iend + 1
            end do

            if (any(SWC_day(ibeg:iend) .le. 0.d0)) then
                print*,"Freeze-thaw error: SWC must be positive in phase days ",ibeg," through ",iend
                stop
            end if
            swc_mean = sum(SWC_day(ibeg:iend)) / DBLE(iend - ibeg + 1)
            if (swc_mean .gt. swc_sat) then
                print*,"Freeze-thaw error: phase-mean SWC exceeds saturation in days ",ibeg," through ",iend
                print*,"phase-mean SWC, saturated SWC = ",swc_mean,swc_sat
                stop
            end if

            SWC_phase(ibeg:iend) = swc_mean
            ibeg = iend + 1
        end do
    END !!subFT_FillPhaseMeanSWC

    !-----------------------------------------------------------------------------
    SUBROUTINE subFT_ComputeSeasonalFronts(STP_day, SWC_phase, is_thaw, swc_sat, &
        half_sand, BD_kgm3, Fdep_final, Tdep_final)
        REAL(8), intent(in) :: STP_day(:), SWC_phase(:)
        LOGICAL, intent(in) :: is_thaw(size(STP_day))
        REAL(8), intent(in) :: swc_sat, half_sand, BD_kgm3
        REAL(8), intent(out) :: Fdep_final(size(STP_day)), Tdep_final(size(STP_day))

        REAL(8), PARAMETER :: state_eps = 1.d-12
        INTEGER :: i
        REAL(8) :: DDF_state, DDT_state, DDF_thaw_start, Fdep_thaw_start
        REAL(8) :: Fdep_previous, Tdep_previous, Fdep_candidate, Tdep_candidate
        LOGICAL :: thaw_complete, phase_start
        ...
        do i = 1, size(STP_day)
            if (is_thaw(i)) then
                phase_start = i .eq. 1
                if (i .gt. 1) phase_start = .not. is_thaw(i - 1)
                if (phase_start) then
                    !! Retain the winter DDF and the frozen-front depth at thaw onset.
                    DDF_thaw_start = DDF_state
                    Fdep_thaw_start = Fdep_previous
                    DDT_state = 0.d0
                    Tdep_previous = 0.d0
                    thaw_complete = DDF_thaw_start .le. state_eps .or. &
                        Fdep_thaw_start .le. state_eps
                    if (thaw_complete) then
                        DDF_state = 0.d0
                        DDT_state = 0.d0
                        Fdep_previous = 0.d0
                    end if
                ...
                if (thaw_complete) then
                    ...
                    !! Positive temperatures consume winter DDF and add DDT.
                    !! Negative temperatures within the detected thaw season add DDF
                    !! but do not erase the accumulated DDT.
                    DDF_state = max(0.d0, DDF_state - STP_day(i))
                    DDT_state = DDT_state + max(STP_day(i), 0.d0)

                    Fdep_candidate = Fdep_thaw_start * &
                        dsqrt(max(0.d0, DDF_state / DDF_thaw_start))
                    Tdep_candidate = fFT_ThawDepth(DDT_state, SWC_phase(i), &
                        swc_sat, half_sand, BD_kgm3)

                    Fdep_final(i) = min(Fdep_previous, Fdep_candidate)
                    Tdep_final(i) = max(Tdep_previous, Tdep_candidate)

                    if (DDF_state .le. state_eps .or. &
                        Tdep_final(i) .ge. Fdep_final(i)) then
                        DDF_state = 0.d0
                        DDT_state = 0.d0
                        Fdep_final(i) = 0.d0
                        Tdep_final(i) = 0.d0
                ...
                phase_start = i .eq. 1
                if (i .gt. 1) phase_start = is_thaw(i - 1)
                if (phase_start) then
                    if (i .gt. 1 .and. .not. thaw_complete) then
                        print*,"Freeze-thaw error: seasonal thaw ended before the fronts met at day ",i - 1
                        stop
                    end if
                    DDF_state = 0.d0
                    DDT_state = 0.d0
                ...
                !! Single-front freezing. Direction is constrained to downward only.
                DDF_state = max(0.d0, DDF_state - STP_day(i))
                Fdep_candidate = fFT_FrozenDepth(DDF_state, SWC_phase(i), &
                    swc_sat, half_sand, BD_kgm3)
                Fdep_final(i) = max(Fdep_previous, Fdep_candidate)
                Tdep_final(i) = 0.d0
            end if

            Fdep_previous = Fdep_final(i)
            Tdep_previous = Tdep_final(i)
        end do
    END !!subFT_ComputeSeasonalFronts

    !-----------------------------------------------------------------------------
    SUBROUTINE subFT_ComputePermafrostFronts(STP_day, SWC_phase, is_thaw, swc_sat, &
        half_sand, BD_kgm3, Fdep_final, Tdep_final)
        REAL(8), intent(in) :: STP_day(:), SWC_phase(:)
        LOGICAL, intent(in) :: is_thaw(size(STP_day))
        REAL(8), intent(in) :: swc_sat, half_sand, BD_kgm3
        ...
        REAL(8), PARAMETER :: state_eps = 1.d-12
        INTEGER :: i
        REAL(8) :: DDF_state, DDT_state, DDT_freeze_start, Tdep_freeze_start
        REAL(8) :: Fdep_previous, Tdep_previous, Fdep_candidate, Tdep_candidate
        LOGICAL :: freeze_complete, phase_start

        Fdep_final = 0.d0
        Tdep_final = 0.d0
        DDF_state = 0.d0
        DDT_state = 0.d0
        DDT_freeze_start = 0.d0
        Tdep_freeze_start = 0.d0
        Fdep_previous = 0.d0
        Tdep_previous = 0.d0
        !! If the series begins in winter, the preceding thaw state is unknown.
        !! Treat that incomplete initial phase as wholly frozen until the first thaw.
        ...
        do i = 1, size(STP_day)
            if (is_thaw(i)) then
                ...
                if (i .gt. 1) phase_start = .not. is_thaw(i - 1)
                if (phase_start) then
                    if (i .gt. 1 .and. .not. freeze_complete) then
                        print*,"Freeze-thaw error: permafrost freezing ended before the fronts met at day ",i - 1
                        stop
                    end if
                    DDF_state = 0.d0
                    DDT_state = 0.d0
                ...
                !! Single-front thawing. Direction is constrained to downward only.
                DDT_state = DDT_state + max(STP_day(i), 0.d0)
                Tdep_candidate = fFT_ThawDepth(DDT_state, SWC_phase(i), &
                    swc_sat, half_sand, BD_kgm3)
                Fdep_final(i) = 0.d0
                ...
                if (i .eq. 1) then
                    freeze_complete = .true.
                else if (is_thaw(i - 1)) then
                    DDT_freeze_start = DDT_state
                    Tdep_freeze_start = Tdep_previous
                    DDF_state = 0.d0
                    Fdep_previous = 0.d0
                    freeze_complete = DDT_freeze_start .le. state_eps .or. &
                        Tdep_freeze_start .le. state_eps
                    if (freeze_complete) then
                        DDF_state = 0.d0
                        DDT_state = 0.d0
                        Tdep_previous = 0.d0
                    end if
                ...
                if (freeze_complete) then
                    ...
                    !! During freezing, negative temperature adds DDF and consumes
                    !! the residual DDT. Short warm spells reverse those degree-day
                    !! states, while the directional front constraints remain active.
                    DDF_state = max(0.d0, DDF_state - STP_day(i))
                    DDT_state = max(0.d0, DDT_state + STP_day(i))

                    Fdep_candidate = fFT_FrozenDepth(DDF_state, SWC_phase(i), &
                        swc_sat, half_sand, BD_kgm3)
                    Tdep_candidate = Tdep_freeze_start * &
                        dsqrt(max(0.d0, DDT_state / DDT_freeze_start))

                    Fdep_final(i) = max(Fdep_previous, Fdep_candidate)
                    Tdep_final(i) = min(Tdep_previous, Tdep_candidate)

                    if (DDT_state .le. state_eps .or. &
                        Fdep_final(i) .ge. Tdep_final(i)) then
                        DDF_state = 0.d0
                        DDT_state = 0.d0
                        Fdep_final(i) = 0.d0
                        Tdep_final(i) = 0.d0
    ...
    END !!subFT_ComputePermafrostFronts

    !-----------------------------------------------------------------------------
    REAL(8) FUNCTION fFT_ThawDepth(DDT, swc_ref, swc_sat, half_sand, BD_kgm3)
        REAL(8), intent(in) :: DDT, swc_ref, swc_sat, half_sand, BD_kgm3
        ...
        kt = fFT_get_kt(swc_ref, swc_sat, half_sand, BD_kgm3)
        fFT_ThawDepth = 100.d0 * dsqrt(max(0.d0, 2.d0 * kt * FT_th * DDT / &
            (FT_Lf * FT_density * max(swc_ref, FT_eps))))
    END !!fFT_ThawDepth

    !-----------------------------------------------------------------------------
    REAL(8) FUNCTION fFT_FrozenDepth(DDF, swc_ref, swc_sat, half_sand, BD_kgm3)
        REAL(8), intent(in) :: DDF, swc_ref, swc_sat, half_sand, BD_kgm3
        REAL(8), PARAMETER :: FT_th = 86400.d0
        REAL(8), PARAMETER :: FT_Lf = 3.337d5
        REAL(8), PARAMETER :: FT_density = 1000.d0
        REAL(8), PARAMETER :: FT_eps = 1.d-6
        ...
        kf = fFT_get_kf(swc_ref, swc_sat, half_sand, BD_kgm3)
        fFT_FrozenDepth = 100.d0 * dsqrt(max(0.d0, 2.d0 * kf * FT_th * DDF / &
            (FT_Lf * FT_density * max(swc_ref, FT_eps))))
    END !!fFT_FrozenDepth

    !-----------------------------------------------------------------------------
    !-----------------------------------------------------------------------------
    REAL(8) FUNCTION fFT_get_kt(swc_liq, swc_sat, half_sand, BD_kgm3)
        REAL(8), intent(in) :: swc_liq, swc_sat, half_sand, BD_kgm3
        ...
        kdry = (0.135d0 * BD_kgm3 + 64.7d0) / (2700.d0 - 0.947d0 * BD_kgm3)
        fFT_get_kt = (ksat - kdry) * Ke + kdry
    END !!fFT_get_kt

    !-----------------------------------------------------------------------------
    REAL(8) FUNCTION fFT_get_kf(swc_liq, swc_sat, half_sand, BD_kgm3)
        REAL(8), intent(in) :: swc_liq, swc_sat, half_sand, BD_kgm3
        ...
        do iday = 1,nday
            ...
            do lp = 1, 24  !!
                ...
                if(k.le.sINI%nHour) then
                    ...
                    sINP%GPP = sINI%SIN(k)
                    sINP%tmp = sINI%STP(k)
                    sINP%Fdep = sINI%Fdep(k)
                    sINP%Tdep = sINI%Tdep(k)
                    sINP%SWC = sINI%SWC(k)
                    sINP%SWP = sINI%SWP(k)
                    sINP%pH  = sINI%SpH(k)
...
!                    rRead(1) = max(0.9,sINI%VARobj_tol(i))
    ...
    SUBROUTINE subMEND(xx, sPAR, sINI, sOUT)
        ...
        INTEGER i
        REAL(8) sumPOM, phi  !!OC1, OC2,, sumENZP
...
!        frMBA_to_POM = (/0.1d0, 0.9d0/)  !! 10% entering POM_Oxidative, 90% entering POM_Hydrolytic
        ...
        if (sINI%iALT_type .eq. 0) then
            sOUT % MNPOOL % ALT_scalar = const_FillValue
        else
            sOUT % MNPOOL % ALT_scalar = fALT_DEC(sOUT%MNPOOL%Fdepth, &
                sOUT%MNPOOL%Tdepth, sINI%soilDepth, sPAR%ALTa, sINI%iALT_type) !for comparison
        end if
        ...
        IF(sINI%iKinetics.le.10) then
            ...
            sOUT % CFLUX % DOM_to_MBA = fV_MM(0, smmPAR, smmINP) !&
                                 !* fALT_D2A(sOUT%MNPOOL%Fdepth, sOUT%MNPOOL%Tdepth ,sINI%soilDepth,sPAR%ALTa,sPAR%ALTb) !qss: mark0306
           ...
           !  else 
                !! fraction of actual total uptake   !!qss: mark0201
            ...
            sOUT % CFLUX % MBA_to_MBD = (1.d0 - phi) * sPAR % VmA2D * sINP % CPOOL % MBA !&
                                !* fALT_A2D(sOUT%MNPOOL%Fdepth, sOUT%MNPOOL%Tdepth ,sINI%soilDepth,sPAR%ALTb)  !!qss: mark0306
            sOUT % CFLUX % MBD_to_MBA = phi * sPAR % VmD2A * sINP % CPOOL % MBD !&
                                !* fALT_D2A(sOUT%MNPOOL%Fdepth, sOUT%MNPOOL%Tdepth,sINI%soilDepth,sPAR%ALTa,sPAR%ALTb)  !!mark0306
        ...
        sOUT % MNPOOL % MBC_SOC = sOUT % CPOOL %  MB / sOUT % CPOOL %  SOM   !2023/8/12 added
        sOUT % MNPOOL % ENZ_MBC = sOUT % CPOOL %  ENZ / sOUT % CPOOL %  MB   !2023/8/12 added
        ...
        IF(.NOT.sINI%Carbon_only) THEN
            ...
            ! write(*,*)sINP%MNPOOL%NH4 + sINP%MNPOOL%NO3
...
!            sOUT%NPOOL%DOMT = sOUT%NPOOL%DOMT + sum(sOUT%NFLUX%ENZNm_to_DOM)
    ...
    SUBROUTINE subMEND_PAR(xx, sPAR, sINI)
        ...
        REAL(8) ALT_scalar      !!mark0306
        REAL(8) ALT_scalar_low  !!mark0306
    
        REAL(8) tp
        REAL(8) wp, wfp !!wp=water matric potential, wfp= water-filled porosity
        REAL(8) pH
        REAL(8) fdep, tdep  !!mark0306
...
!            sPARbase%KsMOM      = xx(6)*xx(7)*0.8  !! MOM decomposition,  *0.8
        ...
        sPARbase%ALTa      = xx(49)
        sPARbase%ALTb      = xx(50)  !!qss: 20241105
        sPARbase%ALTc      = xx(51)
        ...
        sPAR % ALTa = sPARbase % ALTa  !!mark0202
        sPAR % ALTb = sPARbase % ALTb
        sPAR % ALTc = sPARbase % ALTc 
        ...
        if (sINI%iALT_type .eq. 0) then
            sPAR % VdPOM(1) = sPARbase % VdPOM(1)*pH_scalar*tp_scalar*wp_scalar_opt
        else
        ...
        pH_scalar = fpH(sCase,pH)
        if (sINI%iALT_type .eq. 0) then
            sPAR % VdPOM(2) = sPARbase % VdPOM(2)*pH_scalar*tp_scalar*wp_scalar
        else
            ALT_scalar = fALT_DEC(fdep, tdep, sINI%soilDepth, sPAR%ALTa, sINI%iALT_type)
        ...
        pH_scalar = fpH(sCase,pH)
        if (sINI%iALT_type .eq. 0) then
            sPAR % VdMOM = sPARbase % VdMOM*pH_scalar*wp_scalar*tp_scalar
        else
            ALT_scalar = fALT_DEC(fdep, tdep, sINI%soilDepth, sPAR%ALTa, sINI%iALT_type)
            sPAR % VdMOM = sPARbase % VdMOM*pH_scalar*wp_scalar*ALT_scalar
        end if
        ...
        if (sINI%iALT_type .eq. 0) then
            sPAR % Vm = sPARbase % Vm*tp_scalar
        else
            sPAR % Vm = sPARbase % Vm*ALT_scalar
        end if
        ...
        if (sINI%iALT_type .eq. 0) then
            ...
            ALT_scalar = fALT_D2A(fdep, tdep, sINI%soilDepth, &
                sPAR%ALTb, sPAR%ALTc, sINI%iALT_type)
            ALT_scalar_low = fALT_A2D(fdep, tdep, sINI%soilDepth, &
                sPAR%ALTb, sPAR%ALTc, sINI%iALT_type)
            sPAR % VmA2D = sPARbase%Vm*ALT_scalar_low
            sPAR % VmD2A = sPARbase%Vm*ALT_scalar
        end if
        ...
        !    end if
        if (sINI%iALT_type .eq. 0) then
            sPAR % VNup_MB = sPARbase % VNup_MB*tp_scalar
        else
            ALT_scalar = fALT_DEC(fdep, tdep, sINI%soilDepth, sPAR%ALTa, sINI%iALT_type)
            sPAR % VNup_MB = sPARbase % VNup_MB*ALT_scalar
        end if
        ...
        if (sINI%iALT_type .eq. 0) then
            ...
            ALT_scalar = fALT_DEC(fdep, tdep, sINI%soilDepth, sPAR%ALTa, sINI%iALT_type)
            sPAR%VNif = sPARbase%Vnif*ALT_scalar
        end if
        ...
        if (sINI%iALT_type .eq. 0) then
            sPAR % VNit = sPARbase % Vnit*wp_scalar_opt*tp_scalar
        else
            ALT_scalar = fALT_DEC(fdep, tdep, sINI%soilDepth, sPAR%ALTa, sINI%iALT_type)
            sPAR % VNit = sPARbase % Vnit*wp_scalar_opt*ALT_scalar
        end if
        ...
        wp_scalar_opt   = fWFP_PieceWise(sCase,wfp)
        if (sINI%iALT_type .eq. 0) then
            sPAR % VDenit(1) = sPARbase % VDenit(1)*wp_scalar_opt*tp_scalar
        else
            ALT_scalar = fALT_DEC(fdep, tdep, sINI%soilDepth, sPAR%ALTa, sINI%iALT_type)
            sPAR % VDenit(1) = sPARbase % VDenit(1)*wp_scalar_opt*ALT_scalar
        end if
        ...
        ! sPAR % VDenit(2)   = sPARbase % VDenit(2)*ALT_scalar*wp_scalar_opt
        ...
        ! sPAR % VDenit(3)   = sPARbase % VDenit(3)*ALT_scalar*wp_scalar_opt
    ...
    !!-----------------------------------------------------------------
    REAL(8) function fSWC2SWP(SWC0,SWCres,SWCsat,alpha,n)
        !!van-Genuchten equation, SWP in units of [cm], SWC is fraction (0-1)
    ...
    REAL(8) function fO2_scalar(SWC,Porosity)
        ...
        !fO2_scalar = O2_CONC/(O2_CONC + Ks_O2)  !!20250219
        fO2_scalar = max( O2_CONC/(O2_CONC + Ks_O2), 0.4d0)
        return
    ...
    REAL(8) function fracNO3_Leaching(SWC,SWCsat,SWCfc,Ksat,Lambda,rNleach,soil_depth,dt)
    ...
    REAL(8) FUNCTION fALT_DEC(Fdep,Tdep,Depth,ALTa,iALT_type) 
        ...
        SELECT CASE (iALT_type)
        CASE (0)
            fALT_DEC = const_FillValue
        CASE (1)
            ALT = 0.0d0
            ALT_scalar = 0.0d0
        ...
        CASE DEFAULT
            print*,"ALT error: invalid iALT_type = ",iALT_type
            stop
        END SELECT
    ...
    REAL(8) FUNCTION fGasEfflux(Ngas,Gas_Conc_Soil,Porosity, Diff_dist,Altitude, SWCFC, SWC, Temperature, DiffAir0)
        ...
        !!LOCAL VARIABLES:
        REAL(8) DiffAir, DiffSoil, Gas_Conc_Air,fGasEfflux_unit_area  !!pressure,
        ...
        do j = 1,nVAR
            ...
            select case (k)  !!see "MEND.ini"
                ...
                case (41)   !!Active fraction of microbes
                    dSIM(iHour,j) = sOUT%CPOOL%MBA/max(sOUT%CPOOL%MB,1d-6)
                case (42)   !!Total enzyme concerntrations
                    dSIM(iHour,j) = sOUT%CPOOL%ENZD
                case (45)   !!MBC/SOC                       !2023/8/12 added
                    dSIM(iHour,j) = sOUT%MNPOOL%MBC_SOC     !2023/8/12 added
                case (46)   !!ENZ/MBC                       !2023/8/12 added
                    dSIM(iHour,j) = sOUT%MNPOOL%ENZ_MBC     !2023/8/12 added
                case default
                    dSIM(iHour,j) = sOUT%MNFLUX%CO2_gmo
            end select
        end do
    ...
    SUBROUTINE sCOFI_PAR(nPAR, PAR_names, sFile_in,sFile_out, nSA)
        ...
        real(8) fObj0,fObj_cr(2)  !!upper and lower bound for COFI
        real(8) xx(nPAR)
    ...
    SUBROUTINE sSOBOL_VAR_OBJ0(sFile_t0,sFile_t,nRow_skip,nVAR,sDate_beg,sDate_end,tstep,var_names,fobj)
        ...
        REAL(8), ALLOCATABLE:: dSIM_t0(:,:)
        REAL(8), ALLOCATABLE:: dSIM_t(:,:) !!days in 1 month, at most = 31, hours in 1 month, at most =24*31
    ...
    SUBROUTINE sSOBOL_VAR_OBJ(itype,xx, sPAR, sINI, sOUT, sFile_in,sFile_out, &
        ...
        REAL(8), intent(in) :: xx(nPAR)
        REAL(8), intent(out):: Sobol_obj(nVAR)
        ...
        !    CHARACTER(len=3000)var_names
        REAL(8) fobj
        REAL(8) dSIM_t0(nt,nVAR)
        REAL(8) dSIM_t(nt,nVAR)
    ...
    SUBROUTINE sSOBOL_VAR_Read(dSIM_t,sFile_t,nRow_skip,nRow,nVAR)
        ...
        !!ARGUMENTS:
        REAL(8), intent(out)    :: dSIM_t(nRow,nVAR)
    ...
    SUBROUTINE subMEND_Files_Open(sINI)
        ...
        write(sINI%iFout_FLX_hour,'(a10,200a20)')"Hour",(trim(sINI%Name_FLUX(i))//"_C",i=1,const_nFLUX), &
            ...
            (trim(sINI%Name_FLUX(i))//"_N",i=1,const_nFLUX)
        ...
        if(trim(ststep).eq."hourly") then
            do i = 1,nfile
                ...
                do j=1,ndays*24
                    ...
                    if(eof<0) exit
                    rINP(k) = rRead  !!input modify  original: max(0d0, rRead)
                end do
                close(101)
            end do
                ...
                do j=1,ndays
                    ...
                    do lp = 1,24 !!24 hours in 1 day
                        ...
                        if(is_total.eq.1) then
                            rINP(k) = max(0d0,rRead)/DBLE(24) !!input modify
                        else
                            rINP(k) = rRead
                        end if
                    end do
