...
!    character(len=8)    :: ssDate_beg_inp2, ssDate_end_inp2
    ...
    real(8)             :: ST_constant, SM_constant, NH4_constant, NO3_constant, Fdep_constant, Tdep_constant
    real(8)             :: Input_type1_constant, Input_type3_constant
    ...
    DATA Name_MNPOOL  /"CO2","Nmine","Nmine_Free","Nmine_Solid","NH4tot","NH4ads","NH4", &
        "NOx","NO3+NO2","NO3","NO2","NO","N2O","N2", "NGas","MBC/SOC","ENZ/MBC",&
        "Fdepth","Tdepth","ALT_scalar"/
    ...
    DATA Name_PAR     /"fRa","fINP","VP1","VP2","VM","KP1","KP2","KM","Qmax","Kba","Kdes","Kads","Kp2u","Ku2p", &
        ...
        "VNup_MB","VNup_VG","rNleach","bNup_VG","KsNH4_MB","KsNO3_MB","YgN","wNUI", &
    ...
    if(sINI%iALT_type.lt.0 .or. sINI%iALT_type.gt.2) then
        print*,"Namelist Error: iALT_type must be 0 (non-frozen soil), 1 (permafrost), or 2 (seasonally frozen soil)"
        stop
    end if
...
!    sINI%sDate_end_inp2 = ssDate_end_inp2
    ...
    if(sINI%pid.eq.0) then
        ...
        write(sSCE%iFout_ini,*) "*--------Carbon-Nitrogen Coupled MEND---------*"
        write(sSCE%iFout_ini,*) "*------------MPI Parallel Version-------------*"
        write(sSCE%iFout_ini,*) "*----wang.gangsheng@gmail.com; Oct 10, 2019---*"
        write(sSCE%iFout_ini,*) "*---------------------------------------------*"
#ifdef USE_MPI
        ...
        write(sINI%iFout_ITW_hour,'(a10,8a20)')"Hour","SIN_mg/cm3/h","STP_oC","Fdep","Tdep","SWC","SWP_MPa","pH","SIN3_mg/cm3/h"
    end if
    ...
    CASE (0)
        !! Freeze-thaw quantities are not applicable to non-frozen soil.
        !! Fill values are output/state indicators only; subMEND_PAR bypasses ALT regulation.
        sINI%Fdep = const_FillValue
        sINI%Tdep = const_FillValue
    CASE (1,2)
        CALL subFreezeThawFront_Generate(sINI)
    CASE DEFAULT
        !! Defensive guard; the namelist validation above should make this unreachable.
        print*,"Namelist Error: invalid iALT_type = ",sINI%iALT_type
        stop
    END SELECT
    ...
    if(sINI%pid.eq.0) then
        ...
        write(sSCE%iFout_ini,*)"sINI%iALT_type = ", sINI%iALT_type, &
            " (0: non-frozen soil, 1: permafrost, 2: seasonally frozen soil)"
    ...
    if(sINI%pid.eq.0) then
        do k = 1,ndays
            ...
            do j=1,24 !!sINI%nHour
                ...
                i = (k - 1)*24 + j
                write(sINI%iFout_ITW_hour,'(A10,7f20.6)') &
                    sDate//str2,sINI%SIN(i),sINI%STP(i),sINI%Fdep(i),sINI%Tdep(i),sINI%SWC(i),sINI%SWP(i),sINI%SpH(i),sINI%SIN3(i)
            end do
        end do
